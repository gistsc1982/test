    ...
    return {
      ...
      flatNodeList: [
        ...
        { "id": "geojson-usgs-quake","name":"USGS 近30日全球M2.5+地震(GeoJSON)","parentId":"folder-geojson","nodeType":"layer","url":"https://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/2.5_month.geojson","sortOrder":4,"visible":1,"description":"USGS 美国地质调查局，近30天全球震级≥2.5地震事件 GeoJSON。包含 mag(震级)/place(地点)/time(时间)/depth(深度) 等字段。","icon":"🌍","centerLon":116.4,"centerLat":39.9,"centerHeight":15000000,"geoJsonStyle":{"markerColor":"#FF4400","markerSize":24,"fillOpacity":0.5},"pinField":"mag","pinFontSize":16,"pinTextColor":"#FFFFFF","pinPixelOffsetY":-200},
	        ...
	        { "id": "geojson-ne-cities","name":"Natural Earth 全球主要城市点位(GeoJSON)","parentId":"folder-geojson","nodeType":"layer","url":"https://d2ad6b4ur7yvpq.cloudfront.net/naturalearth-3.3.0/ne_50m_populated_places_simple.geojson","sortOrder":7,"visible":1,"description":"Natural Earth 全球约1200个主要城市点位。CloudFront CDN。字段: name(城市)/adm0name(国家)/pop_max(人口)。","icon":"🏙️","centerLon":104,"centerLat":35,"centerHeight":5000000,"geoJsonStyle":{"markerColor":"#3388FF","markerSize":18,"strokeWidth":1,"fillOpacity":0.6},"pinField":"name","pinFontSize":14,"pinTextColor":"#FFFFFF","pinPixelOffsetY":-16},
	        { "id": "geojson-ne-airports","name":"Natural Earth 全球机场点位(GeoJSON)","parentId":"folder-geojson","nodeType":"layer","url":"https://d2ad6b4ur7yvpq.cloudfront.net/naturalearth-3.3.0/ne_10m_airports.geojson","sortOrder":8,"visible":1,"description":"Natural Earth 全球主要机场点位。CloudFront CDN。字段: name(机场)/iata_code(IATA)/type(类型)。","icon":"✈️","centerLon":106,"centerLat":32,"centerHeight":5000000,"geoJsonStyle":{"markerColor":"#FF4400","markerSize":14,"strokeWidth":1,"fillOpacity":0.5},"pinField":"name","pinFontSize":13,"pinTextColor":"#FFFFFF","pinPixelOffsetY":-14},
	        { "id": "geojson-china-cities","name":"中国主要城市点位(GeoJSON·本地)","parentId":"folder-geojson","nodeType":"layer","url":"/data/gis/geojson/china_cities.geojson","sortOrder":9,"visible":1,"description":"从Natural Earth筛选的中国98个主要城市点位。本地文件，加载极快。","icon":"🇨🇳","centerLon":104,"centerLat":35,"centerHeight":5000000,"geoJsonStyle":{"markerColor":"#3388FF","markerSize":18,"strokeWidth":1,"fillOpacity":0.6},"pinField":"name","pinFontSize":14,"pinTextColor":"#FFFFFF","pinPixelOffsetY":-16},
	        { "id": "geojson-china-airports","name":"中国主要机场点位(GeoJSON·本地)","parentId":"folder-geojson","nodeType":"layer","url":"/data/gis/geojson/china_airports.geojson","sortOrder":10,"visible":1,"description":"按ICAO代码筛选的中国46个机场点位(含台港澳)。本地文件，加载极快。","icon":"✈️","centerLon":106,"centerLat":32,"centerHeight":5000000,"geoJsonStyle":{"markerColor":"#FF4400","markerSize":14,"strokeWidth":1,"fillOpacity":0.5},"pinField":"name","pinFontSize":13,"pinTextColor":"#FFFFFF","pinPixelOffsetY":-14},
    ...
    _mergeBuiltinNodes() {
      ...
      var builtinNodes = [
        ...
        { "id": "geojson-datav-beijing","name":"DataV 北京市行政区划(GeoJSON)","parentId":"folder-geojson","nodeType":"layer","url":"https://geo.datav.aliyun.com/areas_v3/bound/geojson?code=110000_full","sortOrder":4,"visible":1,"description":"阿里云 DataV 北京市区县级行政区划边界。阿里云CDN，国内访问极快。","icon":"🏙️","centerLon":116.4,"centerLat":39.9,"centerHeight":80000,"geoJsonStyle":{"stroke":"#FF6644","strokeWidth":2,"fill":"#FF6644","fillOpacity":0.12,"outlineColor":"#CC4422","outlineWidth":2,"markerColor":"#FF6644","markerSize":10}},
	        ...
	        { "id": "geojson-china-cities","name":"中国主要城市点位(GeoJSON·本地)","parentId":"folder-geojson","nodeType":"layer","url":"/data/gis/geojson/china_cities.geojson","sortOrder":7,"visible":1,"description":"从Natural Earth筛选的中国98个主要城市点位。本地文件，加载极快。","icon":"🇨🇳","centerLon":104,"centerLat":35,"centerHeight":5000000,"geoJsonStyle":{"markerColor":"#3388FF","markerSize":18,"strokeWidth":1,"fillOpacity":0.6},"pinField":"name","pinFontSize":14,"pinTextColor":"#FFFFFF","pinPixelOffsetY":-16},
	        { "id": "geojson-china-airports","name":"中国主要机场点位(GeoJSON·本地)","parentId":"folder-geojson","nodeType":"layer","url":"/data/gis/geojson/china_airports.geojson","sortOrder":8,"visible":1,"description":"按ICAO代码筛选的中国46个机场点位(含台港澳)。本地文件，加载极快。","icon":"✈️","centerLon":106,"centerLat":32,"centerHeight":5000000,"geoJsonStyle":{"markerColor":"#FF4400","markerSize":14,"strokeWidth":1,"fillOpacity":0.5},"pinField":"name","pinFontSize":13,"pinTextColor":"#FFFFFF","pinPixelOffsetY":-14}
      ];
...
window.__cesiumViewer__.scene.terrainProvider = terrainProvider;
          ...
          case 'local-terrain-tiles': {
            ...
            // 从 terrain 路径推导 GeoTIFF 路径：/data/dem/terrain/{name} → /data/dem/{name}.tif
            var terrainName = tilesBaseUrl.replace(/\\/g, '/').split('/').filter(Boolean).pop();
            var geotiffUrl = tilesBaseUrl.replace(/\/terrain\/[^/]+$/, '') + '/' + terrainName + '.tif';
            console.log('[LayerTreeManager] 🖼️ 尝试加载对应 GeoTIFF 伪彩色图: ' + geotiffUrl);

            try {
              await this._ensureGeoTiff();
              if (typeof window.GeoTIFF !== 'undefined') {
                var imgResp = await fetch(geotiffUrl, { signal: createTimeoutSignal(30000) });
                if (imgResp.ok) {
                  var imgBlob = await imgResp.blob();
                  var tiff = await window.GeoTIFF.fromArrayBuffer(await imgBlob.arrayBuffer());
                  ...
                  var imgWest, imgEast, imgSouth, imgNorth;
                  ...
                  var imgProvider = new Cesium.SingleTileImageryProvider({
                    url: imageUrl,
                    rectangle: Cesium.Rectangle.fromDegrees(imgWest, imgSouth, imgEast, imgNorth)
                  });
                  ...
                  console.log('[LayerTreeManager] 🖼️ DEM 伪彩色图已叠加到地形 (alpha=0.7, ' +
                    imgWest.toFixed(4) + '°~' + imgEast.toFixed(4) + '°E, ' +
                    imgSouth.toFixed(4) + '°~' + imgNorth.toFixed(4) + '°N, ' +
                    tMin.toFixed(0) + '~' + tMax.toFixed(0) + 'm)');
                } else {
                  console.log('[LayerTreeManager] ℹ️ 未找到对应 GeoTIFF 文件，跳过伪彩色图叠加');
                }
              }
            } catch (e) {
              console.warn('[LayerTreeManager] ⚠️ 加载 DEM 伪彩色图失败: ' + (e?.message || e));
            }
            break;
          }
          case 'local-dem': {
    ...
    flyToLayerNode(node) {
      ...
      try {
        ...
        if (entry && entry._bounds) {
          ...
          // terrain 瓦片粗级全为 0，必须飞得够低才能看到起伏（level 8-10 才有有效数据）
          // 1° × 110km/° × 0.15 ≈ 16.5km，Cesium 自然使用 level 8-9 瓦片
          var flyHeight = Math.max(lonSpan, latSpan) * 110000 * 0.15;
          viewer.camera.flyTo({
