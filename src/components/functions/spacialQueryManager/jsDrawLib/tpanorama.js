...
(function (global, undefined) {
    ...
    function initCamera() {
        _camera = new THREE.PerspectiveCamera(90, window.innerWidth/2 / window.innerHeight, 1, 10000);
        _camera.target = new THREE.Vector3(0, 0, 0);
        _cameraOrtho = new THREE.PerspectiveCamera(90, window.innerWidth/2 / window.innerHeight, 1, 10000);
    ...
    function makePanorama(pRadius, widthSegments, heightSegments, u) {
        mesh = new THREE.Mesh(new THREE.SphereGeometry(pRadius, widthSegments, heightSegments), new THREE.MeshBasicMaterial({ map: THREE.ImageUtils.loadTexture(u) }));
        mesh.scale.x = -1;
        _scene.add(mesh);
    }
    ...
    function initRenderer() {
        ...
        _renderer.setSize(window.innerWidth/2, window.innerHeight);
    ...
    function onDocumentMouseClick(event) {
        _mouse.x = event.clientX / window.innerWidth/2 * 2 - 1;
    ...
    function onWindowResize() {
        _camera.aspect = window.innerWidth/2 / window.innerHeight;
        _camera.projectionMatrix.makePerspective(_fov, _camera.aspect, 1, 1100);
        _camera.updateProjectionMatrix();
        _cameraOrtho.left = -window.innerWidth/2 / 2;
        _cameraOrtho.right = window.innerWidth/2 / 2;
    ...
    function worldPostion2Screen(world_vector, camera) {
        ...
        var result = {
            x: Math.round((vector.x + 1) * window.innerWidth/2 / 2 - window.innerWidth/2 / 2),
