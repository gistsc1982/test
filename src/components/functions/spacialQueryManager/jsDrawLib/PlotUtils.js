...
PlotUtils.getArcPoints = function (center, radius, startAngle, endAngle) {
    ...
    for (var i = 0; i <= L.Constants.FITTING_COUNT; i++) {
        var angle = startAngle + angleDiff * i / L.Constants.FITTING_COUNT;
...
PlotUtils.getBisectorNormals = function (t, pnt1, pnt2, pnt3) {
    ...
    if (dist > L.Constants.ZERO_TOLERANCE) {
...
PlotUtils.getCurvePoints = function (t, controlPoints) {
    ...
    for (i = 0; i < controlPoints.length - 1; i++) {
        ...
        for (var t = 0; t < L.Constants.FITTING_COUNT; t++) {
            var pnt = PlotUtils.getCubicValue(t / L.Constants.FITTING_COUNT, pnt1, normals[i * 2], normals[i * 2 + 1], pnt2);
            points.push(pnt);
        }
        points.push(pnt2);
    }
